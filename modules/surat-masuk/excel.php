...
if (isset($_POST['export'])) {
	...
	if ($tgl_awal == $tgl_akhir) { 
		header("content-disposition: attachment;filename=Laporan_Surat_Masuk_Tanggal_".date('d-m-Y', strtotime($_POST['tgl_awal'])).".xls");
    } else {
		header("content-disposition: attachment;filename=Laporan_Surat_Masuk_Tanggal_".date('d-m-Y', strtotime($_POST['tgl_awal']))."_sd_".date('d-m-Y', strtotime($_POST['tgl_akhir'])).".xls");
    }
	?>
	...
	<table border='1'>
		<h3>
			<thead>
				<tr>
					...
					<td align="center" valign="top"><strong>TANGGAL DITERIMA</strong></td>
					<td align="center" valign="top"><strong>ASAL SURAT</strong></td>
        ...
        $result = $mysqli->query("SELECT a.id_surat_masuk,a.nomor_agenda,a.tanggal_diterima,a.asal_surat,a.nomor_surat,a.tanggal_surat,a.perihal,a.keterangan,b.id_instansi,b.nama_instansi
                                  FROM surat_masuk as a INNER JOIN instansi as b ON a.asal_surat=b.id_instansi
                                  WHERE a.tanggal_diterima BETWEEN '$tgl_awal' AND '$tgl_akhir' ORDER BY a.id_surat_masuk ASC") 
        ...
        if ($rows > 0) {
            while ($data = $result->fetch_assoc()) { ?>
				<tr>
                    ...
                    <td align="center" valign="top"><?php echo date('d-m-Y', strtotime($data['tanggal_diterima'])); ?></td>
